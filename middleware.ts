...
export async function middleware(request: NextRequest) {
  ...
  const protectedRoutes = ['/dashboard', '/kasir', '/settings', '/laporan', '/admin']
  ...
  if (user) {
    ...
    const rules: Record<string, string[]> = {
      ...
      '/dashboard': ['pemilik', 'apoteker'],
    }
    for (const [route, allowed] of Object.entries(rules)) {
      if (pathname.startsWith(route) && !allowed.includes(role)) {
        return NextResponse.redirect(new URL(role === 'kasir_magang' || role === 'kasir_senior' ? '/kasir' : '/dashboard', request.url))
      }
    }
  }
  return response
